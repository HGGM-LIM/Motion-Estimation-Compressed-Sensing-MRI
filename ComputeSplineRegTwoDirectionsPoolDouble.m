...
[Ireg,Grid,Spacing,M,B,F]=image_registration(abs(u0(:,:,2)),abs(u0(:,:,1)),Options);
GridAll     = zeros([size(Grid) numTime]);
SpacingAll  = zeros([size(Spacing) numTime]);
GridDAll    = zeros([size(Grid) numTime]);
SpacingDAll = zeros([size(Spacing) numTime]);

% parfor ih = 1:2*numTime % uncomment to parallelize
for ih = 1:2*numTime
    if ih <= numTime
        % Backward
        Imoving             = abs(u0(:,:,ih));
